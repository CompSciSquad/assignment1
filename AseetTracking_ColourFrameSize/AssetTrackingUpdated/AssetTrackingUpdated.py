...
my_w_1 = Frame(root, bg ="lightcyan1")
my_w_2 = Frame(root, bg = "lightcyan1")
...
label_username = tk.Label(my_w_2, text="Username", anchor="e", bg = "lightcyan1")
label_password = tk.Label(my_w_2, text="Password", anchor="e", bg = "lightcyan1")
...
logBtn = tk.Button(my_w_2, text="Log In", bg="white", command=_login_btn_clicked)
logBtn.grid(columnspan=2)

var1 = IntVar()
checkBtn = tk.Checkbutton(my_w_2, text="Accept terms and conditions", onvalue=1, offvalue=0, variable=var1, bg = "lightcyan1").grid(row=6, column=1, sticky=W)


link = tk.Label(my_w_2, text="Policy Document Link", fg="blue", cursor="hand2", bg = "lightcyan1")
link.grid(row=7, column=1)
link.bind("<Button-1>", callback)

lLogin = tk.Label(my_w_2,  text='Administrator Log In', font=('Helvetica', 12), width=30, anchor="c", bg = "lightcyan1")
lLogin.grid(row=0, column= 1)

canvas = Canvas(my_w_2, width = 200, height = 200, bg = "lightcyan1", highlightthickness = 0)
...
# function to display selected query data on window
def display_query_data(query='select * from User'):
    for index in asset_tree_view.get_children():
        asset_tree_view.delete(index)
    r_set = my_conn.execute(query)
    for row in r_set:
        asset_tree_view.insert('', 'end', values=row)
...
def countdown(time):
    if time == -1:  # if the time reaches -1, terminate the program
        ...
        l4 = tk.Label(my_w_1, text="Time remaining: %d seconds" % time, anchor='w', bg = "lightcyan1")  # show with a label how much time is left
...
def execute_query():
    query = query_search_entry.get()
    display_query_data(query)
...
# add employee label
lSearch = tk.Label(my_w_1,  text='Search Employee', font=('Helvetica', 12), width=30, anchor="c", bg = "lightcyan1")
lSearch.grid(row=0, column=9, columnspan=4)

# ID Label for search
lid = tk.Label(my_w_1,  text='Enter User ID: ', width=15, font = ('bold', 10), anchor = "e", bg = "lightcyan1")
lid.grid(row=1, column=10)

# ID text box for search
tid = tk.Text(my_w_1,  height=1, width=10)
tid.grid(row=1, column=11)

# search user button
b1 = tk.Button(my_w_1, text='Search User', width=10, bg='white', command=lambda: search(tid.get('1.0', END)), font = ('bold', 10))
...
# add employee label
lEmp = tk.Label(my_w_1,  text='Add Employee', font=('Helvetica', 12), width=20, anchor="c", bg = "lightcyan1")
lEmp.grid(row=0, column=6, columnspan=4)

# ID label for add_data()
lid2 = tk.Label(my_w_1,  text='ID: ', width=10, anchor="e", font=('bold', 10), bg = "lightcyan1")
...
lName = tk.Label(my_w_1,  text='Name: ', width=10, anchor="e", font=('bold', 10), bg = "lightcyan1")
...
lsname = tk.Label(my_w_1,  text='Surname: ', width=10, anchor="e", font=('bold', 10), bg = "lightcyan1")
...
lAsset = tk.Label(my_w_1,  text='Asset: ', width=10, anchor="e", font=('bold', 10), bg = "lightcyan1")
...
bAdd = tk.Button(my_w_1,  text='Add Record', width=10, bg="white", command=lambda: add_data())
bAdd.grid(row=5, column=7)

# add update label
lUpdate = tk.Label(my_w_1,  text='Update Asset', font=('Helvetica', 12), width=30, anchor="c", bg = "lightcyan1")
lUpdate.grid(row=10, column=6, columnspan=4)

# ID label for update()
lid2 = tk.Label(my_w_1,  text='ID: ', width=8, anchor="e", font=('bold', 10), bg = "lightcyan1")
...
bUpdate = tk.Button(my_w_1,  text='Update Record', width=12, bg='white', command=lambda: update())
bUpdate.grid(row=14, column=7)

# add delete label
lDelete = tk.Label(my_w_1,  text='Delete Record', font=('Helvetica', 12), width=30, anchor="c", bg = "lightcyan1")
lDelete.grid(row=3, column=9, columnspan=4)

# ID label for delete()
lid3 = tk.Label(my_w_1,  text='ID: ', width=10, anchor="e", font=('bold', 10), bg = "lightcyan1")
...
bDelete = tk.Button(my_w_1,  text='Delete Record', width=10, bg='white', command=lambda: delete())
bDelete.grid(row=5, column=11)

# exit button
bExit = tk.Button(my_w_1, text='Exit', width=8, bg='white', command=lambda: destroy_frames())
